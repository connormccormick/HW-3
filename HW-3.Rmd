...
profiles <- read_csv(file="profiles copy.csv") %>% 
  filter(!is.na(height)) %>% 
...
ggplot(gender, aes(x = as.factor(is_female), y = prop)) +
  ...
  theme_minimal() +
  labs(title = "Proportion by Sex of Sample", x = "Sex", y = "Proportion")  
...
```{r, echo=TRUE, message=TRUE, warning=TRUE, fig.width=8, fig.height=4.5}
#regression
predictsexmodel <- glm(is_female ~ 1 + income + job + petowners, data=training, family="binomial")
...
predictions_tidy <- predictions %>%
  gather(type, fitted_prob, -c(is_female, job, income, petowners))

#How did the model do?
prophecy <- predictions_tidy %>%
  mutate(guess = ifelse(fitted_prob>=.5, 1, 0))

errors <- prophecy %>%
  mutate(correct = ifelse(guess==is_female, 1, 0))
  
propwrong <- errors %>%
  summarize(1 - mean(correct))
kable(propwrong)
...
```{r, echo=TRUE, message=TRUE, warning=TRUE, fig.width=8, fig.height=4.5}
test <- test %>% 
  select(income, job, is_female, petowners) %>%
  mutate(phat=predict(predictsexmodel, newdata = test, type = "response"))

testpredictions_tidy <- test %>%
  gather(type, fitted_prob, -c(is_female, job, income, petowners))

#How did the model do for the test?
testprophecy <- testpredictions_tidy %>%
  mutate(guess = ifelse(fitted_prob>=.5, 1, 0))
...
schoolyard <- graveyard %>%
  ...
  tally() %>% 
  rename(playcount = n) %>%
  filter(!playcount < 228) %>%
  unite(title, artist, track, sep = ":", remove = FALSE)

ggplot(schoolyard, aes(x=fct_reorder(title, playcount), y=playcount)) +
  geom_bar(stat = "identity", fill = "darkslateblue") +
  coord_flip()+
  labs(title="Most Popular Songs during the Graveyard Shift", x='Track Name', y='Play Count')
